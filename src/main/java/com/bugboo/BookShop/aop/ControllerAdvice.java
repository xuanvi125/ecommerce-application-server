...
//    }
    ...
    ResponseEntity<ErrorApiResponse> handleConstraintViolationException(ConstraintViolationException e){
        ErrorApiResponse response = new ErrorApiResponse();
        response.setStatus("fail");
        response.setMessage(e.getConstraintViolations().iterator().next().getMessage());
        return ResponseEntity.badRequest().body(response);
    }
